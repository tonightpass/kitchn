...
First, you need to import the `useRect` hook from your custom hooks file.
...
|----------------|-------------------------------------------------|---------------------------------------------------
| `initialState` | `ReactiveDomReact` or `() => ReactiveDomReact`  | (Optional) Initial state for the rectangle dimensions and position.
...
| `setRect`        | `(event: React.MouseEvent<HTMLElement> or React.FocusEvent<HTMLElement>, getContainer?: () => HTMLElement or null)` | A function to update the position and dimensions of the tracked element.
| `setRectWithRef` | `(React.MutableRefObject<HTMLElement or null>, getContainer?: () => HTMLElement or null)` | A function to update the position and dimensions of the tracked element using a ref.
